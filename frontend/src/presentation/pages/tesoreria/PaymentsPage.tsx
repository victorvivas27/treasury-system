...
import { FiCheck, FiClipboard, FiCreditCard, FiExternalLink, FiUpload, FiX } from "react-icons/fi";
...
export const PaymentsPage = () => {
  ...
  const upload = async (installmentId: number, file?: File) => {
    if (!file) return; const form = new FormData(); form.append("file", file); setBusy(true);
    try { await apiClient.post(`${base}/mis-cuotas/${installmentId}/comprobante`, form); await load(); flash("Comprobante enviado"); }
    catch { setError("No se pudo enviar. Usa un JPG, PNG o PDF dentro del límite permitido."); } finally { setBusy(false); }
  };
