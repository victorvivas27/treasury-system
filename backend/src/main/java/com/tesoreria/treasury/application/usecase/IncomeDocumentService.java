...
public class IncomeDocumentService {
    private static final Logger LOGGER = LoggerFactory.getLogger(IncomeDocumentService.class);
    ...
    private final IncomeDocumentJpaRepository documents;
    private final FileStorageService storage;
    private final TreasuryUseCase treasury;
    private final long maxBytes;

    public IncomeDocumentService(IncomeDocumentJpaRepository documents, FileStorageService storage,
    ...
    @Transactional
    public DocumentView upload(Long incomeId, MultipartFile file, String user) {
        TreasuryIncome income = treasury.getIncome(incomeId);
        ValidatedFile valid = validate(file);
        String objectName = "tesorerias/%d/ingresos/%s_%d_%s.%s".formatted(
                income.schoolYear(), sanitizedBaseName(valid.originalName(), valid.extension()),
                income.id(), UUID.randomUUID(), valid.extension());
        storage.upload(objectName, valid.bytes(), valid.contentType());
        try {
            LocalDateTime now = LocalDateTime.now();
            IncomeDocumentEntity entity = new IncomeDocumentEntity();
            entity.setTreasuryYear(income.schoolYear());
            entity.setIncomeId(income.id());
            ...
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("INCOME_DOCUMENT_UPLOADED documentId={} treasuryYear={} incomeId={} object={} result=success",
                        saved.id(), income.schoolYear(), income.id(), objectName);
            }
            return saved;
    ...
    public List<DocumentView> list(Long incomeId) {
        TreasuryIncome income = treasury.getIncome(incomeId);
        return documents.findAllByTreasuryYearAndIncomeIdOrderByCreatedAtDesc(
                income.schoolYear(), income.id()).stream().map(this::view).toList();
    }

    @Transactional(readOnly = true)
    public Download download(Long incomeId, Long documentId) {
        IncomeDocumentEntity document = owned(incomeId, documentId);
    ...
    @Transactional
    public void delete(Long incomeId, Long documentId) {
        IncomeDocumentEntity document = owned(incomeId, documentId);
        storage.delete(document.getStorageObjectName());
        documents.delete(document);
    }

    private IncomeDocumentEntity owned(Long incomeId, Long documentId) {
        TreasuryIncome income = treasury.getIncome(incomeId);
        return documents.findByIdAndTreasuryYearAndIncomeId(
                documentId, income.schoolYear(), income.id())
    ...
    private DocumentView view(IncomeDocumentEntity value) {
